...
The full syntax for all of the attributes that are available to the |resource package_ips| resource is:
...
* ``accept_license``, ``options``, ``package_name``, ``provider``, ``source``, ``timeout``, and ``version`` are attributes of this resource, with example values shown. |see attributes|
