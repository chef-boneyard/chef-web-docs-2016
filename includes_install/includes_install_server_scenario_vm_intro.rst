...
The following example walks through the process of setting up the open source version of |chef server 11| in a virtual machine. Any type virtual machine software can be used to run the |chef server|; however, this example is based on the following assumptions:
...
* The |chef server| will be accessed from a browser that is hosted on the same computer on which the virtual machine is located
