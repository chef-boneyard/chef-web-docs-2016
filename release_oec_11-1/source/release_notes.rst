...
* **Front-end certificate service** The certificate service for public and private key generation is moved to the front-end for non-standalone installations. This allows this service to scale horizontally. 
* **Bookshelf hostname configuration** The host for the location in which cookbooks are stored---bookshelf---is now configurable. Previous versions of |chef server oec| directed this traffic directly to the backend host:port of the bookshelf service. .
* **Bookshelf traffic travels over HTTPS** |chef server oec| defaults to the host header set by the incoming HTTP request, ensuring that URLs generated by bookshelf from requests to the API front end will be directed back to that front-end and will be correctly proxied to the back end.
