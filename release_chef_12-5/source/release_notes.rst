...
* **Use custom resources instead of providers and definitions** The new way of defining custom resource changes the ways that providers and definitions are used with |chef|. Defining a provider (in the ``/providers`` directory) is no longer necessary. The typical purpose of a definition (in the ``/definitions`` directory) is to create a reusable component that can be leveraged by recipes across any number of cookbooks: use a custom resource instead of a definition. 
