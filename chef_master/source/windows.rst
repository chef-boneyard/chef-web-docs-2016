...
* To ensure the |resource powershell_script| resource is idempotent, six helpers are present in the |dsl recipe| to help verify the registry as a script runs: ``registry_data_exists?``, ``registry_get_subkeys``, ``registry_get_values``, ``registry_has_subkeys?``, ``registry_key_exists?``, and ``registry_value_exists?``
