...
.. list-table::
   ...
   * - :doc:`essentials_cookbook_files_syntax`
     - The |resource cookbook_file| resource defines a file distribution.
   ...
   * - :doc:`essentials_cookbook_files_transfers`
     - When a |resource cookbook_file| resource is run, the |chef client| will calculate the checksum of each local file and then compare this against the checksum for that same file as it currently exists in the cookbook on the |chef server|.
