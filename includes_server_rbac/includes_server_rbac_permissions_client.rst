...
Permissions for nodes that will be configured by the |chef client| should have the ``DELETE``, ``GRANT``, ``READ`` and ``UPDATE`` permissions and also belong to the ``clients`` group.
