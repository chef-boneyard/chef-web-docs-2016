...
The following user accounts exist on the |chef server|: ``pivotol`` (a superuser account), ``alice``, ``bob``, ``carol``, and ``dan``. Run the following command to view a list of users on the |chef server|:
