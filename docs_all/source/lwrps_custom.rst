...
.. note:: The Cron cookbook is used by many |chef| organizations and represents a fairly stable set of functionality that is used safely by many users of |chef|. That said, and because many lightweight resource are customized for specific |chef| organizations (e.g. appear in cookbooks that are only used by a single |chef| organization) the attributes contained within them are specific to how nodes will be managed by |chef| for that organization. Other lightweight resources appear in cookbooks that are shared among the |chef| community and while these cookbooks will operate successfully across organizations, it's important to take steps to ensure that any custom element (such as a lightweight resource or lightweight provider) is taking the steps required by your organization.
...
.. warning:: DRAFT: This section needs to also be a DSL section as pure "reference"?
...
.. warning:: MIGHT NOT NEED THIS, BUT IT IS AN EXAMPLE OF USING THE :callbacks VALIDATION PARAMETER
...
updated_by_last_action Method -- DONE?
+++++++++++++++++++++++++++++++++++++++++++++++++++++
xxxxx
...
whyrun_supported? Method -- DONE
+++++++++++++++++++++++++++++++++++++++++++++++++++++
.. START_INCLUDE; CHECK essentials_node_why_run AND HOOK IN SAME THERE
...
Doug Ireton's blog has a nice series on lightweight resources:
