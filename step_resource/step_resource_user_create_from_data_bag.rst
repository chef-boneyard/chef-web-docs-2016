...
where ``users`` is the name of the data bag, each "user" (``user_name``) is the name of a data bag item, and then for each user the |resource user| resource creates the username/password. See the `page for the user resource <http://docs.getchef.com/resource_user.html>`_ for more information about additional attributes that can be used to define other aspects of a user, such as ``shell``, ``system``, and so on.