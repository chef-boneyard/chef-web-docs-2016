...
``-p FILE``, ``--public-key FILE``
   Set the initial public key for the client from a file on disk (cannot be passed with ``--prevent-keygen``).
...
``-k``, ``--prevent-keygen``
   ...
   On Chef 12.1 server or later, clients can be created without an initial public key.
   Use this option to create a client without a public key. Keys for this client can be managed later be added via the ``knife client key`` subcommand.
   If neither this option nor ``--public-key`` are passed, a Chef 12.1+ server will create a new user with a public key named "default" and return the private key.
   If only ``--public-key`` is passed, a Chef 12 server will use the public key in the file you passed as the "default" public key for the new user.
   On servers before 12.1, this option is not valid, and a public key will always be generated by the server for a new user unless ``--public-key`` is passed, in which case the passed key will be used instead.
